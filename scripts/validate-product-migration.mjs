...
const refreshMigrationPath = process.argv[4];
if (!migrationPath || !expected) {
  throw new Error("Usage: node scripts/validate-product-migration.mjs <migration> <expected-count>");
}
...
try {
  ...
  const countProducts = async (imageOnly = false) => {
    let total = 0;
    for (let index = 0; index < styleNumbers.length; index += 80) {
      const chunk = styleNumbers.slice(index, index + 80);
      const placeholders = chunk.map(() => "?").join(",");
      const row = await db
        .prepare(`SELECT COUNT(*) count FROM products WHERE style_number IN (${placeholders})${imageOnly ? " AND image_url LIKE '/catalog/shoes/%'" : ""}`)
        .bind(...chunk)
        .first();
  ...
  const first = await countProducts();

  for (const statement of statements) await db.prepare(statement).run();
  const afterRerun = await countProducts();
  if (first !== expected || afterRerun !== expected) {
    throw new Error(`Unexpected counts: first=${first}, afterRerun=${afterRerun}`);
  }

  if (refreshMigrationPath) {
    const refreshSource = await readFile(refreshMigrationPath, "utf8");
    const refreshStatements = refreshSource
      .split("--> statement-breakpoint")
      .map((statement) => statement.trim())
      .filter(Boolean);
    for (const statement of refreshStatements) await db.prepare(statement).run();
    const refreshed = await countProducts(true);
    if (refreshed !== expected) {
      throw new Error(`Expected ${expected} refreshed products, found ${refreshed}`);
    }
  }

  console.log(JSON.stringify({ styleNumbers, products: first, afterRerun, refreshed: Boolean(refreshMigrationPath) }));
} finally {
  await mf.dispose();
}
